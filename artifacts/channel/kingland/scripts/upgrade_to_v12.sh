...
addCapabilityToChannel() {
  ...
  if [ $GROUP == "application" ]; then
    ...
    for ((i=1; i<=4; i++))
    do
         signConfigtxAsPeerOrg $i config_update_in_envelope.pb
    done
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer1.kingland.com:7050 --cafile $ORDERER_CA
    res=$?
    set +x
  else
    set -x
    peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer1.kingland.com:7050 --tls true --cafile $ORDERER_CA
    res=$?
    set +x
  fi

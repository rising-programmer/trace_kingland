...
function printHelp () {
  echo "Usage: "
  echo "  kingland.sh up|down|restart|generate [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>]"
  echo "  kingland.sh -h|--help (print this message)"
  ...
  echo
  echo "	kingland.sh generate -c mychannel"
  echo "	kingland.sh up -c mychannel -s couchdb"
  echo "	kingland.sh up -l node"
  echo "	kingland.sh down -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	kingland.sh generate"
  echo "	kingland.sh up"
  echo "	kingland.sh down"
}
...
function networkUp () {
   ...
   # generate yaml through template
    ...
    if [ -d "../crypto-config/peerOrganizations/org${ORG_NAME_NUMBER}.kingland.com" ];then
  ...
  if [ ! -d "crypto-config" ]; then
    ...
    createConfigTx
    #copy material
    rm -rf ../crypto-config/peerOrganizations/org${ORG_NAME_NUMBER}.kingland.com
    cp -r crypto-config/peerOrganizations/org${ORG_NAME_NUMBER}.kingland.com ../crypto-config/peerOrganizations/
  ...
  cd ../
    sh network_setup.sh createPeerTemplate -f docker-compose-peer${ORG_NAME_NUMBER}.yaml -o org${ORG_NAME_NUMBER} -M Org${ORG_NAME_NUMBER}MSP -p peer0.org${ORG_NAME_NUMBER}.kingland.com -c couchdb${ORG_NAME_NUMBER} -P ${COUCHDB_PORT} -l ${PEER_LISTEN_PORT} -e ${PEER_EVENT_PORT}
    sh network_setup.sh up -f docker-compose-peer${ORG_NAME_NUMBER}.yaml

    sh network_setup.sh createCATemplate -f docker-compose-ca-org${ORG_NAME_NUMBER}.yaml -o org${ORG_NAME_NUMBER}.kingland.com -P ${CA_PORT}
...
function upgradeNetwork() {
  ...
  docker inspect -f '{{.Config.Volumes}}' orderer1.kingland.com | grep -q '/var/hyperledger/production'
  if [ $? -ne 0 ]; then
